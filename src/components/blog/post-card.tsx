...
export function PostCard({
  ...
  return (
    <Link
      href={`/blog/${post.slug}`}
      className={cn(
        "group flex h-full flex-col overflow-hidden rounded-2xl border border-line transition-[border-color,box-shadow] duration-300 hover:border-accent/40 hover:shadow-soft",
        featured ? "bg-fg text-bg" : "bg-bg-elevated",
      )}
    >
      {post.cover && (
        <div className="aspect-[3/2] shrink-0 overflow-hidden border-b border-line bg-[#f6f3ed]">
          <Image
            src={post.cover}
            alt={post.coverAlt ?? ""}
            width={1536}
            ...
            className="h-full w-full object-contain transition-transform duration-500 motion-safe:group-hover:scale-[1.025]"
          />
        </div>
      )}
      <div className="flex flex-1 flex-col p-5 sm:p-6">
        ...
        <div className={cn("mt-auto flex items-center justify-between gap-3 border-t pt-4 font-mono text-[11px]", featured ? "border-bg/20 text-bg/65" : "border-line text-muted")}>
          ...
          <span aria-hidden="true" className={cn("grid h-8 w-8 shrink-0 place-items-center rounded-full transition-colors duration-300 group-hover:bg-accent group-hover:text-white", featured ? "bg-bg/10 text-bg" : "bg-bg text-fg")}>
